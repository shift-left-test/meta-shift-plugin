...
@customElement('branch-coverage-recipes')
/**
 * branch coverage recipes.
 */
export class BranchCoverageRecipes extends PagedTable {
  /**
   * constructor.
   */
  constructor() {
    ...
    this.columns = [
      ...
      {title: 'Branches', field: 'total', widthGrow: 1,
        formatter: this.localeNumberString.bind(this)},
      {title: 'Covered', field: 'first', widthGrow: 1,
        formatter: this.localeNumberString.bind(this)},
      {title: 'Uncovered', field: 'second', widthGrow: 1,
        formatter: this.localeNumberString.bind(this)},
  ...
  _handleRowClicked(e, row) {
    window.location.href = `../${row.getData().name}/branch_coverage`;
  }
}
