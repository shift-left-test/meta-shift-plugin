...
@customElement('complexity-recipes')
/**
 * branch coverage recipes.
 */
export class ComplexityRecipes extends PagedTable {
  /**
   * constructor.
   */
  constructor() {
    ...
    this.columns = [
      ...
      {title: 'Functions', field: 'total',
        width: Constants.IssueCountWidth + 10,
        formatter: this.localeNumberString.bind(this)},
      {title: 'Abnormal', field: 'first',
        width: Constants.IssueCountWidth + 10,
        formatter: this.localeNumberString.bind(this)},
      {title: 'Normal', field: 'second',
        width: Constants.IssueCountWidth + 10,
        formatter: this.localeNumberString.bind(this)},
  ...
  _handleRowClicked(e, row) {
    window.location.href = `../${row.getData().name}/complexity`;
  }
}
